...
class Externs:
    ...
    _do_raise_keyboardinterrupt_on_union = bool(
        os.environ.get("_RAISE_KEYBOARDINTERRUPT_IN_CFFI_UNION", False)
    )
    ...
    def is_union(self, input_type):
        ...
        if self._do_raise_keyboardinterrupt_on_union:
        ...
        try:
            ...
            if isinstance(res, Get):
                ...
                return self.lib.PyGeneratorResponseGet(
                    res.product, res.subject_declared_type, res.subject,
                )
...
class Native(metaclass=SingletonMetaclass):
    ...
    def new_scheduler(
        ...
        engine_types = self.lib.PyTypes(
            ...
            directories_to_merge=DirectoriesToMerge,
            directory_with_prefix_to_strip=DirectoryWithPrefixToStrip,
            directory_with_prefix_to_add=DirectoryWithPrefixToAdd,

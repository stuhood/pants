...
class SchedulerService(PantsService):
    ...
    def __init__(
        ...
        *,
        ...
        self._invalidation_snapshot: Optional[Snapshot] = None

    def _get_snapshot(self, poll: bool) -> Optional[Snapshot]:
        ...
        try:
            snapshot = self._scheduler_session.product_request(
                Snapshot,
                subjects=[PathGlobs(self._invalidation_globs)],
                poll=poll,
                timeout=timeout,
            )[0]
    ...
    def setup(self, services):
        ...
        self._invalidation_snapshot = self._get_snapshot(poll=False)
        self._logger.info("watching invalidation patterns: {}".format(self._invalidation_globs))
    ...
    def add_invalidation_glob(self, glob: str):
        ...
        # Check one time synchronously with our current set of globs, and then extend the list and reset.
        self._check_invalidation_globs(poll=False)
        self._invalidation_globs.append(glob)
        self._invalidation_snapshot = self._get_snapshot(poll=False)

    def _check_invalidation_globs(self, poll: bool):
        """Check the digest of our invalidation Snapshot and exit if it has changed."""
        assert self._invalidation_snapshot is not None

        snapshot = self._get_snapshot(poll=poll)
        if (
            snapshot is not None
            and snapshot.directory_digest != self._invalidation_snapshot.directory_digest
        ):
